...
<p align="center">
  <strong>Anthropic Claude Code 的开源替代 — 拒绝黑盒</strong><br/>
...
Open Agent 是一个开源 AI 编程智能体框架，基于 **Bun** 和 **TypeScript** 构建，旨在成为 **Anthropic Claude Code（agent SDK）的完全透明替代方案**。每一行系统提示词、工具实现和对话逻辑都是开放且可审计的 — 拒绝黑盒。

项目提供了**交互式 CLI** 和**编程 SDK**，让你拥有同等的智能体能力，同时完全掌控行为、成本和数据流向。

### 路线图

| 阶段 | 重点 | 状态 |
|------|------|------|
| **第一阶段** | 完整的 Claude Code 替代方案 — CLI、SDK、多模型、工具、MCP、权限、会话 | **进行中** |
| **第二阶段** | 多智能体协同 — 团队工作流、规划、大规模智能体间协调 | 计划中 |
| **第三阶段** | 长任务执行 — 持久化智能体、复杂项目执行、自主工作流 | 计划中 |

### 为什么选择 Open Agent？

- **拒绝黑盒** — 每一条系统提示词、工具定义和决策路径都是开源且可审计的。
- **直接替代** — 匹配 Claude Code 的核心能力（工具、MCP、会话、权限），提供兼容的 SDK。
